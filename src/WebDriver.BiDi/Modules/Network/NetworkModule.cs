...
public sealed class NetworkModule(Broker broker) : Module(broker)
{
    ...
    internal async Task ContinueWithAuthAsync(Request request, ContinueWithnullAuthOptions? options = null)
    {
        await Broker.ExecuteCommandAsync(new ContinueWithAuthCommand(new ContinueWithnullAuth(request)), options).ConfigureAwait(false);
    }
